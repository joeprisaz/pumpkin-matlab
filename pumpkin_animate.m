...
%% ANIMATE
close all; f = figure('visible', 'on', 'Color', 'w', 'Position', [1 1 15 15]);
x = 0:pi/50:2*pi;
w = 0.3*sin(x) + 1;
% h = -0.3*sin(x) + 1;
% h = -0.3*sin(x) + 1;
...
v = VideoWriter('pumpkin_circle2.mp4','MPEG-4'); v.FrameRate = 25; v.Quality = 100; open(v)

for ii = 1:length(x)
    [Xp,Yp,Zp,Cp,Xs,Ys,Zs] = pumpkinFunction(w(ii),h(ii));
    surf(Xp,Yp,Zp,Cp,'Clipping','off');
    shading interp
    colormap([ linspace(.94, 1, 256); linspace(.37, .46, 256); linspace(0, .1, 256) ]');
    surface(Xs,Ys,Zs,[],'FaceColor', '#008000', 'EdgeColor','none', 'Clipping','off');
    material([ .6, .9, .3, 2, .5 ])
    lighting g
    axis('equal',1.5*[-1 1 -1 1 -1 1],'off')
    camlight
...
%     pause(0.01)
    ...
    F = getframe(f,[295 325 526 506]); writeVideo(v,F)
end
